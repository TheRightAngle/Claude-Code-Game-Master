    ...
    def search_plots(self, query: str) -> Dict[str, Dict]:
        ...
        for name, data in plots.items():
            ...
            if isinstance(npcs, list) and any(
                isinstance(npc, str) and query_lower in npc.lower() for npc in npcs
            ):
                results[name] = data
                continue
            ...
            if isinstance(locations, list) and any(
                isinstance(loc, str) and query_lower in loc.lower() for loc in locations
            ):
                results[name] = data
                continue
            ...
            if isinstance(objectives, list) and any(
                isinstance(obj, str) and query_lower in obj.lower() for obj in objectives
            ):
                results[name] = data
                continue
